...
def NER_RELATION(text, tokenizer, ner_model, max_len=256):
    ...
    with torch.no_grad():
        if ner_model.use_crf:
            scores = ner_model(input_ids, attention_mask, token_type_ids)
...
if __name__ == '__main__':
    all_ = []
    for d in tqdm(json.load(open('./datasets/CHIP-STS/CHIP-STS_test.json'))):
        all_.append(NER_RELATION(d["text"], tokenizer=tokenizer, ner_model=model))
    json.dump(
        all_,
        open('./outputs/CHIP-STS_test.json', 'w'),
        indent=4,
        ensure_ascii=False
    )